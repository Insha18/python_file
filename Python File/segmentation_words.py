...
def get_words(raw_image):
    ...
    img_for_det = cv2.rotate(img_for_det, cv2.ROTATE_180)
    img_for_ext = cv2.rotate(img_for_ext, cv2.ROTATE_180)
    ...
    for i in range ( 0, len(ycoords)-1 ): #iterate line
        ...
        line = img_for_det[list(range( int(ycoords[i]),int(ycoords[i+1])))]
        ...
        for j in range (0, len(xcoords)-1 ): #iterate words
            ...
            line = img_for_ext[list(range( int(ycoords[i]), int(ycoords[i+1])))]
