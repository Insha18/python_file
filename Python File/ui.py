...
class Window(QtWidgets.QMainWindow):
	...
	#Quit Button Right
		quitbtn = QtWidgets.QPushButton("Quit",self)
	...
	#Select Image
		loadimgbtn = QtWidgets.QPushButton("Load image",self)
	...
	#Crop Image
		cropimgbtn = QtWidgets.QPushButton("Crop image",self)
	...
	#Extract Image
		extracttxtbtn = QtWidgets.QPushButton("Extract text",self)
	...
	#File location QLabel
		loc_qlabel = QtWidgets.QLabel(self)
		loc_qlabel.hide()
		[self.loc_qlabel_signal.connect(x) for x in [lambda: loc_qlabel.setText(str(self.location_name)), lambda: loc_qlabel.show()]]
		loc_qlabel.move(1070,270)
		#self.loc_qlabel_signal.connect(lambda: loc_qlabel.show())
	
	#Crop boundary QLabel
		img_bound_qlabel = QtWidgets.QLabel(self)
	...
	#Vertical Box Right
		v_but_box = QtWidgets.QVBoxLayout()
	...
	#Insert OCR image	
		...
		pic.setGeometry(200, 20, 830, 100)
	...
	def extract_without_crop_message(self):
		extract_choice = QtWidgets.QMessageBox.question(self,"Continue?","Are you sure you want to continue without cropping the image?",QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
		if extract_choice == QtWidgets.QMessageBox.Yes:
	...
	def load_image(self):
		...
		name = QtWidgets.QFileDialog.getOpenFileName(self,"Select image","C:\\Users\Anupam\Desktop\SemV Project\SemV Project","Image files (*.jpg *.gif *.png)")
		print(name[0])
		self.loc_qlabel_signal.emit()
		input_img = cv2.imread(str(name[0]))
		...
		qImg = QtGui.QImage(cv2.cvtColor(resized_img, cv2.COLOR_BGR2RGB), res_width, res_height, res_width*3, QtGui.QImage.Format_RGB888)
		pix = QtGui.QPixmap(qImg)
		inputimg_label = QtWidgets.QLabel(self)
		inputimg_label = QtWidgets.QLabel(self)
